...
{
    mlx->mlx_coin = mlx_xpm_file_to_image(mlx->mlx_init, "./textures/mlx_coin.xpm", &mlx->w, &mlx->h);
    mlx->mlx_player = mlx_xpm_file_to_image(mlx->mlx_init, "./textures/mlx_player.xpm", &mlx->w, &mlx->h);
    mlx->mlx_exit = mlx_xpm_file_to_image(mlx->mlx_init, "./textures/mlx_exit.xpm", &mlx->w, &mlx->h);
    mlx->mlx_wall = mlx_xpm_file_to_image(mlx->mlx_init, "./textures/mlx_wall.xpm", &mlx->w, &mlx->h);
    mlx->mlx_ground = mlx_xpm_file_to_image(mlx->mlx_init, "./textures/mlx_ground.xpm", &mlx->w, &mlx->h);
    if (!mlx->mlx_coin || !mlx->mlx_player || !mlx->mlx_exit || !mlx->mlx_wall || !mlx->mlx_ground)
        ft_error();
}
...
void    ft_put_images(t_game mlx)
{
    ...
    while (mlx.map[i])
    {
        ...
        {
            if (mlx.map[i][j] == 'C')
                mlx_put_image_to_window(mlx.mlx_init, mlx.mlx_window, mlx.mlx_coin, j * 80, i * 80);
            else if (mlx.map[i][j] == 'P')
                mlx_put_image_to_window(mlx.mlx_init, mlx.mlx_window, mlx.mlx_player, j * 80, i * 80);
            else if (mlx.map[i][j] == 'E')
                mlx_put_image_to_window(mlx.mlx_init, mlx.mlx_window, mlx.mlx_exit, j * 80, i * 80);
            else if (mlx.map[i][j] == '1')
                mlx_put_image_to_window(mlx.mlx_init, mlx.mlx_window, mlx.mlx_wall, j * 80, i * 80);
            else if (mlx.map[i][j] == '0')
                mlx_put_image_to_window(mlx.mlx_init, mlx.mlx_window, mlx.mlx_ground, j * 80, i * 80);
            j++;
        }
        i++;
    }
...
void ft_moveup(t_game *mlx, int *collectibles)
{
    ...
    if (mlx->map[mlx->pos.i - 1][mlx->pos.j] == '0')
        ft_swap(&mlx->map[mlx->pos.i][mlx->pos.j], &mlx->map[mlx->pos.i - 1][mlx->pos.j]);
...
void ft_moveright(t_game *mlx, int *collectibles)
{
    ...
    if (mlx->map[mlx->pos.i][mlx->pos.j + 1] == '0')
        ft_swap(&mlx->map[mlx->pos.i][mlx->pos.j], &mlx->map[mlx->pos.i][mlx->pos.j + 1]);
...
void ft_game(char **map)
{
    ...
    mlx.mlx_window = mlx_new_window(mlx.mlx_init, (info.col + 1) * 80, (info.r + 1) * 80, "so_long");
